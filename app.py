...
def processRequest(req):
    ...
    if intent == 'covid_searchcountry':
        ...
        webhookresponse = "***Covid Report*** \n\n" + " New cases :" + str(fulfillmentText.get('new')) + \
            ...
            deaths_data.get('new')) + \
            "\n" + " Total Test Done : " + \
            str(deaths_data.get('total')) + "\n\n*******END********* \n "
        print(webhookresponse)
        log.saveConversations(sessionID, cust_country,
                              webhookresponse, intent, db)
        ...
        log.saveConversations(sessionID, query_text,
                              fulfillmentText, intent, db)
    elif intent == "send_report_to_email":
        fulfillmentText = result.get("fulfillmentText")
        log.saveConversations(sessionID, "Sure send email",
                              fulfillmentText, intent, db)
        ...
        print(webhookresponse)
        log.saveConversations(sessionID, "Cases worldwide",
                              webhookresponse, intent, db)
        ...
        log.saveConversations(sessionID, "Indian State Cases",
                              webhookresponse1, intent, db)
        return {
...
def configureDataBase():
    client = MongoClient(
        "mongodb+srv://admin:Naruto123@cluster0.pmmlm.mongodb.net/covid chatbot?retryWrites=true&w=majority")
